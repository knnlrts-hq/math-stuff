 ...
 * from the work, or will reject if the worker throws an unhandlederror.
 */
 ...
 * perPixel we simply render whatever portion of the Mandelbrotset fits in
...
class MandelbrotCanvas {
    ...
    // This function makes a change to the PageState, then rerenders the
    ...
    // second argument is false, then the new state will not besaved. (We
    ...
    render() {
        ...
        this.pendingRender = Promise.all(promises).then(responses => {
                ...
                if (min === max) { // If all the pixels are the same,
                    ...
                    // In the normal case where min and max aredifferent, use a
